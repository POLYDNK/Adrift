    ...
    void LateUpdate()
    {
        ...
        if (numOfCharacters != currNumOfChars)
        {
            ...
            foreach (Character unit in battleScript.units)
            {
                ...
                {
                    if (unit == icon.GetComponent<CharacterIcon>().myChar)
                    {
                        isNewIcon = false;
                    }
                }
                ...
                if (isNewIcon == true)
                {
                    ...
                    // Set Icon Data
                    CharacterIcon iconScript = newIcon.GetComponent<CharacterIcon>();
                    iconScript.myChar = unit;
                    newIcon.transform.GetChild(0).GetComponent<Image>().sprite = iconScript.myChar.GetComponent<Character>().icon;

                    // Position based on whether ally or enemy
                    if (iconScript.myChar.GetComponent<Character>().IsPlayer())
                    {
                        // Add icon to list
                        ...
                        newIcon.transform.GetChild(2).GetChild(0).GetComponent<RectangleBar>().gradient = Character.HealthBarGradient(true);
                    ...
                    else
                    {
                        ...
                        newIcon.transform.GetChild(2).GetChild(0).GetComponent<RectangleBar>().gradient = Character.HealthBarGradient(false);
